...
class DynaCool(VisaInstrument):
    ...
    def __init__(
        ...
        self.temperature: Parameter = self.add_parameter(
            ...
            label="Temperature",
            unit="K",
            get_parser=partial(DynaCool._pick_one, 2, float),
            get_cmd="TEMP?",
        )
    ...
    def _measured_field_getter(self) -> float:
        resp = self.ask('FELD?')
        number_in_oersted = cast(float, DynaCool._pick_one(2, float, resp))
    ...
    def _temp_getter(
        ...
        """
        Query last temperature setpoint (settling, rate, setpoint) from the instrument.
        """
        raw_response = self.ask("TEMP?")
        mode = DynaCool._pick_one(0, int, raw_response)
        rate = DynaCool._pick_one(1, float, raw_response)
        sp   = DynaCool._pick_one(2, float, raw_response)
